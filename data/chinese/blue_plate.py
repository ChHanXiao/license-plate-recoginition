...
class Draw:
    def __init__(self):
        ...
        self._bg = [
            cv2.resize(cv2.imread(os.path.join(os.path.dirname(__file__), f"res/blue/blue_{imgnum}.png")), (440, 140)) for imgnum in np.arange(0,16)
            ]

    def __call__(self, plate, bg_type=Blue_Type.COMMON):
